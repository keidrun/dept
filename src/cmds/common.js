...
const promiseAllSerially = promises =>
  promises.reduce((prevPromise, currentPromise) => prevPromise.then(currentPromise), Promise.resolve())

const promiseAllParallelly = promises => Promise.all(promises.map(promise => promise()))
...
export { prettyLog, promiseAllParallelly, promiseAllSerially, PACKAGE_MANAGERS, initConfigFile }
